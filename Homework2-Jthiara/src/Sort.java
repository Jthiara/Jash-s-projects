...
public class Sort {
    ...
    public static void merge (int[] data, int low, int high, int k) {
        ...
        if (high < low + k) {
            ...
            Comparable[] subArray2 = new MergesortHeapNode[k];
            int[] outputArr = new int[high + 1]; //helper array
            int[] tempARR2 = new int[k]; //array to track indexes within sub arrays
            
            for (int i = 0, j = low; i < subArray2.length; i++, j++) {
            	int indexIterator = i * (high-low + 1)/ k;
            	subArray2[i] = new MergesortHeapNode(data[low + indexIterator], i); // pass element and index of subarray.
            }
            
            BinaryHeap heap = BinaryHeap.buildHeap(subArray2);
            
            
            for (int j = low; j <= high; j++) { // iterate for length of portioned array
            	try {
            		
            		MergesortHeapNode tempNode = (MergesortHeapNode) heap.deleteMin();
            		outputArr[j] = tempNode.getKey();
            		
            		int nextNum = low + (tempNode.getWhichSubarray() * (high-low+1)/k) + tempARR2[tempNode.getWhichSubarray()]++; // next num in sub array
            		
            		//insert next number from sub array
            		if (nextNum < low + (tempNode.getWhichSubarray() + 1) * (high-low+1)/k) { // compares that num is between low and index of next sub array
            			heap.insert(new MergesortHeapNode(data[nextNum], tempNode.getWhichSubarray())); //place into our heap
            		}
            ...
            //put sorted list back into original array
